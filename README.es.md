...
- Los miembros de grupo que no son líderes pueden abrir "Mi estado de actividad de grupo" desde Actividades de grupo. Reutiliza el mismo componente estructurado que la ventana de reset del líder, pero muestra solo la parte superior de actividad/transporte y la fila del propio jugador. Con clientes compatibles de EZOCore y LibGroupBroadcast muestra el tipo de actividad del raid leader, el objetivo estable resuelto, la etapa y el resultado; la sesión y el TTL recibidos identifican la ejecución y descartan estado obsoleto. En caso contrario informa del estado local y de la preparación del transporte.
...
- Mientras el raid leader está agrupado, los cambios del ciclo de reset se ofrecen a EZOCore como actualizaciones compactas e informativas de `activityState`. Solo EZOCore es propietario del protocolo `EZO_CORE_GROUP_V1` (ID temporal de prueba beta `511`) y del evento de solicitud `EZO_CORE_GROUP_REQUEST_V1` (evento temporal de prueba beta `39`). Las actualizaciones se deduplican y se refrescan con un intervalo limitado antes de que venza su TTL. Como LibGroupBroadcast está limitado al grupo, no puede entregarse ninguna actualización mientras el disband del reset haya dejado a los jugadores fuera de un grupo común; el intercambio se reanuda para los miembros conforme se reconstruye el grupo.
...
- Instala en ambos clientes la misma beta de EZOCore con protocolo v2 y la misma compilación de EZOTools. Inicia un reset controlado como líder y abre Mi estado de actividad de grupo como miembro agrupado. Comprueba que el panel reciba la clave/nombre de la trial, modo Normal/Veterano, progreso exacto y pendientes; no debe fabricar progreso desde la etapa ni usar la dificultad local del miembro. Verifica la zona del líder y el estado misma/distinta instancia consultados localmente, la caducidad TTL, la caché al reabrir el panel y los estados posteriores a reconstruir el grupo. Comprueba que no anuncie actualizaciones durante el hueco provocado por el disband intencionado y que cerrar el panel no afecte al grupo, viajes, invitaciones ni reset. Repite sin EZOCore para verificar el fallback local.
