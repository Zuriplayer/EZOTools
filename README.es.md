...
- Abre Información del grupo en el líder y en un miembro sin una sesión de reset. Comprueba que ambos muestren el mismo tamaño, zona del líder, dificultad efectiva y roster completo, sin progreso, pendientes, mensajes de espera remota ni filas duplicadas. Después instala en ambos clientes la misma beta de EZOCore con protocolo v2 y la misma compilación de EZOTools, inicia un reset controlado como líder y abre el panel en ambos clientes. Comprueba que el líder vea el estado local del reset y que el miembro reciba la clave/nombre de la trial, modo Normal/Veterano, progreso exacto y pendientes; el panel del miembro no debe fabricar progreso desde la etapa ni usar su dificultad local. Verifica la caducidad TTL, la caché al reabrir y los estados posteriores a reconstruir el grupo. Comprueba que cerrar el panel no afecte al grupo, viajes, invitaciones ni reset. Repite sin EZOCore para verificar el fallback local.
