...
- Non-leader group members can open "My group activity status" from Group Activities. It reuses the same structured status-panel component as the leader reset window, but only shows the top activity/transport area and the local player's own row. With compatible EZOCore and LibGroupBroadcast clients, it displays the raid leader's activity type, resolved stable target, stage and result; the received session and TTL identify the run and expire stale state. Otherwise it reports local status and transport readiness.
...
- While the raid leader is grouped, reset lifecycle changes are offered to EZOCore as compact informational `activityState` updates. EZOCore alone owns protocol `EZO_CORE_GROUP_V1` (temporary beta test ID `511`) and request event `EZO_CORE_GROUP_REQUEST_V1` (temporary beta test event `39`). Updates are deduplicated and refreshed at a bounded interval before their TTL expires. Because LibGroupBroadcast is group-scoped, no update can be delivered while the reset's disband phase has left players outside a common group; sharing resumes for members as the group is rebuilt.
...
- The member-facing group activity panel is informational. Received state cannot execute remote commands, accept invitations, change the group, or show the full roster. The separate optional member-travel setting is a local reaction to validated leader state, not a remote command: it is disabled by default, makes at most one jump request per activity session, does not manipulate ESO's native travel prompt, and cannot guarantee that the server completes the requested jump.
...
- Install the same EZOCore protocol-v2 beta and EZOTools build on both clients. Start a controlled reset as leader and open My Group Activity Status as the grouped non-leader. Confirm that the panel receives the trial key/name, Normal/Veteran mode, exact phase progress and pending count; the panel must not fabricate progress from the stage or use the member's local difficulty. Verify the locally queried leader zone and same/different-instance status, TTL expiry, cached state when reopening the panel, and later states after the group is rebuilt. Confirm that no updates are claimed during the intentional disband gap and that closing the panel does not affect group, travel, invitations or reset state. Repeat without EZOCore to verify the local fallback.
