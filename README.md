...
- Leave group, Leave instance, and Leave group and instance are always available from the main EZOTools settings panel (LAM) and quick menus, even during combat. These quick actions are independent from Instance Reset and leader tools.
...
- Verify Leave group as both leader and non-leader and Leave group and instance when both conditions are true; both must remain in Group Activities. Verify Leave instance where ESO allows immediate exit and confirm that it appears only in the main command panel. Repeat in mouse, keyboard-menu, and gamepad-menu flows.
